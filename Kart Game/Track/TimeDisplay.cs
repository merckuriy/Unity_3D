...
namespace KartGame.Track
{
    ...
    public class TimeDisplay : MonoBehaviour
    {
        ...
        [Tooltip("Display the position.")]
        public TimeDisplayItem postionText;
        ...
        private GameFlowManager m_GameManager;
        private ArcadeKartExt kart;
        private int KartsCount;

        private void Start()
        {
            m_GameManager = FindObjectOfType<GameFlowManager>();
            kart = m_GameManager.GetPlayerKart();
            KartsCount = m_GameManager.GetAgentKarts().Length;
            if(KartsCount == 0) {
                KartsCount = 1;
            }
        }
        ...
        void Update()
        {
            if (SceneParameters.gameMode == MyGameMode.Laps) {
                postionText.SetText(DisplayCurrentPosition());
            }

            if (currentLapStartTime == 0) return;
            if (lapsOver || m_GameManager.isGameEnded) return;
        ...
        string DisplayCurrentPosition()
        {
            return kart?.getPosition() + "/" + KartsCount;
        }
